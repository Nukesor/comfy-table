...
pub fn smart_pad_content(
    ...
    for column_index in 0..display_infos.len() - 1 {
        ...
        println!(
            ...
            column_index, padding_needed
        );

        // try padding to the right of the left column
        let mut padding_done = update_column_padding(
            content,
            &mut display_infos,
        ...
        if padding_done > 0 {
            println!(
                "smartpad: column {} padding right {} max_padding {:?}",
                column_index,
                padding_done,
                max_padding_widths[column_index + 1]
            );
        }
        ...
        // try padding to the left of the right column
        padding_done = update_column_padding(
            content,
            &mut display_infos,
            &mut max_padding_widths,
            column_index + 1,
        ...
        if padding_done > 0 {
            println!(
                "smartpad: column {} padding left {} max_padding {:?}",
                column_index + 1,
                padding_done,
                max_padding_widths[column_index + 1]
            );
